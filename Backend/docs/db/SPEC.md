...
> **⚠️ 현재 미사용 (정의만 존재).** 현 CF 알고리즘(`cf_module`)은 본 테이블을 **읽지도 쓰지도 않으며**,
> 유저-음식 신호는 원천 로그(`UserInteractionLog` ⨝ `RecommendationSession`)에서 직접 계산한다.
>
> **⚠️ 태그축 SUM 금지.** 아래 _갱신 방식_을 그대로 구현한 뒤 `total_weight`를 **태그 축으로 SUM 하여
> "유저-음식" 스칼라로 쓰면, 세션에서 선택한 태그 개수에 비례해 가중치가 부풀려진다**(태그 개수 인플레이션).
> 같은 음식을 1번 골라도 태그 2개 선택 시 +4, 3개 선택 시 +6이 되는 식.
> - **유저-음식** 친밀도가 필요하면 → `UserInteractionLog` (태그와 무관) 에서 계산할 것.
> - 본 테이블은 **(유저, 음식, 태그) 단위 "태그 의도별" 신호로만** 사용하고, 태그 축을 SUM으로 무너뜨리지 말 것.
>   (부득이 유저-음식 스칼라가 필요하면 태그 축은 SUM 대신 MAX/AVG 또는 태그 수로 정규화.)
