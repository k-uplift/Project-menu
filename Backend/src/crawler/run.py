    ...
    try:
        with browser_session() as page:
            for i, (mgtno, name, address) in enumerate(sources, 1):
                ...
                verbose = (i == 1)
                cand = find_place_id(
                    name, page=page, source_address=address, verbose=verbose
                )
                ...
                print(f"  → store_id={store_id} (place_id={place_id}, 매칭='{cand['name']}'), "
                      f"메뉴 {len(detail.menus)}, 영업시간 {len(detail.hours)}")
                success += 1
                consecutive_fail = 0
                time.sleep(THROTTLE)

        print(f"\n[완료] 성공 {success}건, place_id 못찾음 {skipped_no_pid}건")
    finally:
        conn.close()
